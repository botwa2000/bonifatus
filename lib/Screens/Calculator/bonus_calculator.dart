...
import 'logged_in_calculator_view.dart';

class BonusCalculator extends StatelessWidget {
    ...
    return FutureBuilder<String?>(
      ...
      builder: (context, snapshot) {
        if (snapshot.hasData && snapshot.data != null) {
          return LoggedInCalculatorView(userId: snapshot.data!);
        } else {
          return const GuestCalculatorView();
        }
      },
    );
  }
...
class GuestCalculatorView extends StatefulWidget {
  const GuestCalculatorView({super.key});

  @override
  State<GuestCalculatorView> createState() => _GuestCalculatorViewState();
}

class _GuestCalculatorViewState extends State<GuestCalculatorView> {
...
// Updated class factor method
  ...
  void _updateCalculations() {
    ...
    setState(() {
      totalGrades = validGrades.toDouble();
      averageScore = validGrades > 0 ? totalGradePoints / validGrades : 0;
      percentageScore = validGrades > 0 ? totalPercentage / validGrades : 0;
      bonusPoints = _calculateBonusPoints();
    });
  }

  double _calculateBonusPoints() {
    double total = 0;
    for (var subject in selectedSubjects) {
      if (subject.grade != null) {
        final grade = Grade.fromJson(subject.grade!);
        total += grade.multiplier * (isFullTerm ? 1.0 : 0.5);
      }
    }
    return total;
  }
